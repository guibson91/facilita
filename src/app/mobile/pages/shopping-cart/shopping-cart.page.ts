...
import { Filial, CardType, CardFoodType, CARDS, CARDS_FOOD } from '../../../../models/filial';
import { Entry, PaymentMethod } from '../../../../models/order';
import { CouponOrder, CouponFilial, Coupon } from '../../../../models/coupon';
import { FormGroup, FormControl, Validators } from '@angular/forms';
import { elementIsInsideArray, getDistanceBetweenPoints } from '../../../util/util';
import { NavController, ToastController, ModalController, LoadingController, AlertController, PopoverController, IonContent } from '@ionic/angular';
...
export class ShoppingCartPage implements OnInit {
  ...
  hasCard(): boolean {
    ...
    return elementIsInsideArray(PaymentMethod.credit_card, this.filial.paymentMethods) || elementIsInsideArray(PaymentMethod.debit_card, this.filial.paymentMethods);
  }

  hasMoney(): boolean {
    if (!this.filial.paymentMethods) return false;
    return elementIsInsideArray(PaymentMethod.money, this.filial.paymentMethods);
  }
  ...
  async verifyCoupon() {
    ...
    loading.present();
    if (this.filial) {
      Filial.coupons.list(this.filial.id).pipe(first()).subscribe((coupons_filial: CouponFilial[]) => {
        let founded: boolean = false;
        coupons_filial.forEach(coupon_filial => {
          if (coupon_filial.enable && coupon_filial.name == this.couponInput) {
            founded = true;
            //Se o cupom não estiver vencido
            if (coupon_filial.finished_at.toDate() >= new Date()) {
              User.coupons.list(this.shared.user.id).pipe(first()).subscribe((coupons_user: Coupon[]) => {
  ...
  hasFoodStamps(): boolean {
    if (!this.filial.paymentMethods) return false;
    return elementIsInsideArray(PaymentMethod.food_stamps, this.filial.paymentMethods);
  }

  hasCreditCard(): boolean {
    if (!this.filial.paymentMethods) return false;
    return elementIsInsideArray(PaymentMethod.credit_card, this.filial.paymentMethods);
  }

  hasDebitCard(): boolean {
    if (!this.filial.paymentMethods) return false;
    return elementIsInsideArray(PaymentMethod.debit_card, this.filial.paymentMethods)
  }
  ...
  async tryFinishOrder() {
    ...
    this.shared.user$.pipe(first()).subscribe(async (user) => {
      ...
      if (this.filial.orderMin && (this.subTotal + this.delivery) < this.filial.orderMin) {
        let minOrder: string = this.filial.orderMin.toFixed(2).replace('.', ',');
        const alert = await this.alertCtrl.create({
          header: "Atenção",
  ...
  async finishOrder() {
    ...
    let difference: string = (this.subTotal - this.auxSubtotal).toFixed(2).replace('.', ',');
      ...
      buttons: [{
        ...
        handler: () => {
          ...
          if (this.auxSubtotal) {
            ...
            this.system.showAlert('Pedido cancelado', null, 'O valor do seu pedido não foi alterado.');
          }
        }
      }, {
        text: 'Confirmar',
        handler: async () => {
          console.log("1");
          this.system.logEvent('ecommerce_purchase', { value: this.cart.totalPrice + this.delivery })
          let loading = await this.loadingCtrl.create({});
          loading.present()
          // Tudo certo com o pedido atual, então tentar efetuar o pedido
          console.log("2");
          this.ngZone.run(() => {
            console.log("3");
            this.cart.send(this.observation, this.delivery, this.coupon).subscribe(async () => {
              loading.dismiss();
  ...
  locationIsValid() {
    ...
    if (this.cart.filial.deliveryByDistrict) {
      ...
      this.cart.filial.deliveryDistricts.forEach(deliveryDistrict => {
        if (deliveryDistrict.location.state_short == this.shared.user.location.state_short &&
          deliveryDistrict.location.city == this.shared.user.location.city &&
          deliveryDistrict.location.district == this.shared.user.location.district) {
  ...
  get orderValidation() {
    ...
    if (this.paymentMethod) {
      if (this.isMoney) {
        ...
        if (this.paymentMethod == this.paymentType.credit_card || this.paymentMethod == this.paymentType.debit_card) {
          if (this.cardFlag) {
            valid = true;
          } else {
